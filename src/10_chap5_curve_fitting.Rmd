...
In Exercise #4, we examined the question of how high to build structures for flood protection, based on the analysis of @Van_Dantzig1956-qp.  In that exercise, we identified the dike height that minimizes the total costs from dike construction and future flooding.  We also examined how changing some of the input parameters within reasonable ranges affects the estimated optimal dike height.
...
In this exercise, we'll fit a second-order polynomial to the sea level data from @Jevrejeva2008-zw, both manually and using automated methods.  We'll also examine the residuals that are left over after performing this curve fit.
